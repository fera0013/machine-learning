...
def display_evaluation_statistics(agents,figure_name='stats'):
    num_axis=int(math.sqrt(len(agents)))
    f, axarr = plt.subplots(num_axis,num_axis)
    f.suptitle(figure_name)
    for i,agent in enumerate(agents):
        y=i%num_axis
        x=(i-y)/num_axis
        axarr[x, y].plot(agent.successful_trials)
        axarr[x, y].plot(agent.driving_errors)
        axarr[x,y].set_title('alpha: '+str(agent.alpha)+' gamma: '+str(agent.gamma))
        axarr[x,y].legend(['#succesfull','#driving errors'])
    plt.show()
...
def run_with_q_learning():
    ...
    for alpha in np.arange(0.1,0.9,0.2):
        for gamma in np.arange(0.1,0.9,0.2):
            ...
            sim.run(n_trials=100) 
            agents.append(a)
    display_evaluation_statistics(agents,"100 trials for all parameter combinations with Q-learning")
def run():
